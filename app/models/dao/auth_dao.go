...
func LoginExec(req *requests.Dang_nhap, res *responses.Dang_nhap) (uint, error) {
	...
	result == 0 {
		return 0, errors.New("ten dang nhap khong ton tai")
	}

	if !helpers.CheckPasswordHash(req.Mat_khau, nhan_vien.Mat_khau) {
		return 0, errors.New("sai mat khau")
	}
	...
	if err := helpers.GormDB.Debug().Table("quyen").
		...
		Find(&ds_quyen).Error; 
	err != nil {
		return 0, errors.New("khong the tim thay quyen")
	}
